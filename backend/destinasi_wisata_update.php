...
/** Menerima data yang akan diubah **/
    $destinasiKODE = $_GET["ubahdestinasi"];
    $edit = mysqli_query($conn, "SELECT * FROM destinasi WHERE destinasi_KODE = '$destinasiKODE' ");
...
/** Mengecek apakah tombol simpan sudah di pilih/klik atau belum **/
	...
	if ($namafoto == "") {
        mysqli_query($conn, "UPDATE destinasi 
            SET destinasi_NAMA = '$destinasiNAMA', 
                destinasi_ALAMAT = '$destinasiALAMAT', 
                destinasi_KET = '$destinasiKET', 
                kecamatan_KODE = '$kecamatanKODE' 
            WHERE destinasi_KODE = '$destinasiKODE'");
    } else {
        mysqli_query($conn, "UPDATE destinasi 
            SET destinasi_NAMA = '$destinasiNAMA', 
                destinasi_ALAMAT = '$destinasiALAMAT', 
...
<div class="row mb-3">
    ...
    <div class="col-sm-10">
        <select class="form-control" id="kecamatanKODE" name="kecamatanKODE">
            ...
            <?php 
                if(mysqli_num_rows($datakecamatan) > 0) { 
                    while($row = mysqli_fetch_array($datakecamatan)) { 
            ?>
